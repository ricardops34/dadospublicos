...
export class Painel360AdminComponent implements OnInit {
  ...
  readonly colunasLotes: PoTableColumn[] = [
    { property: 'arquivoExibicao', label: 'Arquivo', width: '30%' },
    {
      property: 'status',
      label: 'Status',
      type: 'label',
      ...
      labels: [
        ...
        { value: 'cancelado', label: 'Cancelado', color: 'color-03' },
      ],
    },
  ...
  readonly colunasResultados: PoTableColumn[] = [
    ...
    { property: 'razaoSocial', label: 'Razao social', width: '28%' },
    { property: 'cidade', label: 'Cidade', width: '16%' },
    { property: 'uf', label: 'UF', width: '8%' },
    {
      ...
      width: '14%',
      labels: [
        { value: 'sucesso', label: 'Sucesso', color: 'color-10' },
        { value: 'processado', label: 'Processado', color: 'color-10' },
        { value: 'erro', label: 'Erro', color: 'color-05' },
        { value: 'pendente', label: 'Pendente', color: 'color-08' },
      ],
    },
    { property: 'mensagem', label: 'Mensagem', width: '18%' },
  ];
  ...
  carregarLotes(loteIdSelecionado?: string) {
    ...
    this.adminService.listarPainel360Lotes().subscribe({
      next: (lotes) => {
  ...
  selecionarLote(lote: Painel360LoteView) {
    ...
    forkJoin({
      resultados: this.adminService.listarPainel360Resultados(lote.id, 1, 200),
      geoJson: this.adminService.obterPainel360GeoJson(lote.id),
    }).subscribe({
  ...
  onUploadSuccess(event: any) {
    ...
    this.notif.success('Lote enviado para processamento.');
    this.carregarLotes(loteId);
  }

  onUploadError() {
    this.notif.error('Erro ao enviar arquivo para o Painel 360.');
  }

  baixarLote(lote: Painel360LoteView) {
    this.adminService.baixarPainel360Resultado(lote.id).subscribe({
      next: (response) => {
