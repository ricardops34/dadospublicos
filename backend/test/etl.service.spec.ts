...
test('carregarCsv trata quebra de linha dentro de campo entre aspas em estabelecimentos', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'etl-estab-multiline-'));
  const csvPath = path.join(tempDir, 'K3241.K03200Y0.D60110.ESTABELE');
  fs.writeFileSync(
    csvPath,
    '"12345678";"0001";"99";"1";"NOME COM\nQUEBRA";"02";"20240101";"00";"";"000";"20240101";"0111301";"";"RUA";"A";"10";"COMPL";"CENTRO";"78000000";"MT";"9067";"0081";"12345678";"";"";"0081";"12345678";"EMAIL@TESTE.COM";"";"20240101"\n',
    'latin1',
  );
  ...
  assert.equal(total, 1);
  assert.equal(executed.length, 1);
  assert.equal(executed[0].params[4], 'NOME COM\nQUEBRA');
