...
test('minhas busca notificacoes pessoais e broadcast em ordem decrescente com limite de 30', async () => {
  const notificacoes = [
    { id: 'n1', titulo: 'Pessoal', clienteId: 'cli-1' },
    { id: 'n2', titulo: 'Broadcast', clienteId: null },
  ];
  ...
  const result = await service.minhas('cli-1');

  assert.deepEqual(result, notificacoes);
  assert.equal(calls.find.length, 1);
  assert.equal(calls.find[0].where[0].clienteId, 'cli-1');
...
test('contarNaoLidas soma notificacoes pessoais e broadcast ainda nao lidas', async () => {
  ...
  const total = await service.contarNaoLidas('cli-1');

  assert.equal(total, 4);
  assert.equal(calls.count.length, 1);
  assert.equal(calls.count[0].where[0].clienteId, 'cli-1');
...
test('marcarTodasLidas nao persiste nada quando nao houver notificacoes pendentes', async () => {
  ...
  await service.marcarTodasLidas('cli-1');
...
test('marcarTodasLidas persiste notificacoes pessoais marcadas como lidas', async () => {
  const pendentes = [
    { id: 'n1', clienteId: 'cli-1', lida: false, titulo: 'Conta' },
    { id: 'n2', clienteId: 'cli-1', lida: false, titulo: 'Financeiro' },
  ];
  ...
  assert.deepEqual(calls.save[0], [
    { id: 'n1', clienteId: 'cli-1', lida: true, titulo: 'Conta' },
    { id: 'n2', clienteId: 'cli-1', lida: true, titulo: 'Financeiro' },
  ]);
});

test('criar salva notificacao broadcast quando clienteId nao for informado', async () => {
  const { service, calls } = createService({
  ...
  assert.equal(result.clienteId, null);
});
